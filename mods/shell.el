...
;;
;; * zsh/tcsh-style directory tracking and stack management
;;
;;   Features:
...
;;   really makes moving around in the shell more pleasant. But
...
(defvar shell-home-aliases (if (boundp 'user-home-directory)
...
(defalias 'dirs 'my/shell-resync-dirs)
    ...
    (cond ((zerop (length arg))
	  ...
	  (t
	   ...
	   (let* ((old-wd default-directory)
		 ...
		 (setq shell-dirstack
                       (-remove (lambda (d) (f-same? ex-dir d)) shell-dirstack))))
	   (shell-dirstack-message)))))
...
(add-my-hook shell-mode-hook
  ...
  ;;
  ;; Include cwd on the mode line (with ~ and abbreviations as appropriate)
  ;;
                  ...
                  (let ((cwd (abbreviate-file-name default-directory)))
                    (propertize
                     ...
                     'local-map (make-mode-line-mouse-map
                                 'down-mouse-1 `(lambda () ; ATTN mouse-1 pre lion, down-mouse-1 on lion
                                                  (interactive)
                                                  (dired ,cwd)))
